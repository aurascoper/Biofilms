...
Parcel centres are rejection-sampled uniformly over $r < R - 4$, which at $N = 60$, $R = N/2 = 30$
is a disk of radius $26 = 0.867R$. Its expected mean radius is $2(26)/3 = 17.3$ lattice units with
a per-parcel standard deviation of $26/\sqrt{18} = 6.1$. At $n = 6$ parcels the standard error on a
species mean is $\approx 2.5$ lattice units, so the 4.5-unit gap between \textit{C.~neoformans} and
\textit{B.~subtilis} is $\approx 1.3$ standard errors of the difference, and \emph{both} species
means sit within one standard error of the seeding mean ($0.578R$). No multi-seed run against that
