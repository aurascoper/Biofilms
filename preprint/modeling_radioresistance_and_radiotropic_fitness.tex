...
the dynamics, so the two are not separately identifiable. A single-seed radial pattern runs
...
accepted moves, not of attempts.}
...
The direct radiation term reversed no accepted move at either length, and across seeds 42, 43 and
44 at 400~MCS it reversed none of 206\,042. This is the one-part-in-$10^5$ bias of
Table~\ref{tab:term_magnitudes} expressed as a count rather than a ratio, and it follows from it:
$\beta_{s,\mathrm{ion}}$ is negative for two of the seven species at $-5\times10^{-5}$, so the
largest radiation contribution that could favour an accepted move is of that order, and reversing
the move would require the drawn variate to fall within about $10^{-5}$ of the acceptance
threshold. The melanin term reversed 23 moves in 53\,603. Both figures restate the closing sentence
above --- the hand-specified adhesion and volume terms are larger than either radiation-derived
term --- and neither is a measurement of anything outside this model's own acceptance arithmetic.
...
settings cited in Section~1 are closer to $\mathrm{mGy}\,\mathrm{yr}^{-1}$, so a sensitivity
...
revision \texttt{c7539a9}; this revision is prepared against \texttt{f72aabb}. Each names the
...
stratification'' where Section~7 reports a single-seed diagnostic not separated from the seeding
...
regenerated. Figures 3 and 4 still carry a ``per cent depleted'' label that should read
non-penetration; the coupled path that produces them is now gated, so the label is corrected in
the generator and not yet in the artifact. The defect was that the figure generator had been
corrected two weeks earlier and the committed images had not, with no check able to detect the
difference; this is recorded as \texttt{FIG-01} to \texttt{FIG-04} in the claims ledger.
