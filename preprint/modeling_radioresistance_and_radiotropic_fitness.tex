...
measured results. At the melanin level reached in the single seed-42 run, the melanin term
changes Metropolis acceptance by approximately 15.5\%, about an order of magnitude beyond the
reach of the direct per-species radiation term, which for one role of the two negatively signed
species is only one part in $10^5$;\footnote{Corrected 2026-09-15. This sentence stated the
melanin figure without the seed it was computed at, and set it against the one-role radiation
value rather than the term's reach (\texttt{data/claims\_ledger.csv}: \texttt{PP-62-13},
\texttt{PP-65-06}).} only the product of melanin production and melanin coupling reaches the
dynamics, so the two are not separately identifiable. A single-seed radial pattern runs
...
simulations---two Langevin models, the CPM, and a radial radiodialysis solver---realise only
...
integrator requires; the word survives in a comment where the physics does not. Implementing
genuine symplectic integration is listed in Section~\ref{sec:limitations}.
...
Section~\ref{sec:results} for the seed-42 run is this coefficient's effect: the largest measured quantity in the
...
in Section~3 is a proposed organising formalism and is not discretised by any of these programs.
...
operations. The active Metropolis rule contains four terms,
...
constants $I_0=1.0$ and $T_{\mathrm{CPM}}=5.0$. The melanin row is evaluated at the
\textit{C.~sphaerospermum} field value of the single seed-42 run; that value, and the bias computed
from it, varies between seeds.}
\label{tab:term_magnitudes}
\begin{tabular}{@{}llr@{}}
\toprule
\textbf{Term} & $\Delta H$ & \textbf{Acceptance bias} \\
\midrule
$\Delta H_{\mathrm{rad}}$, negative $\beta_{\mathrm{ion}}=-5\times10^{-5}$ & $-5.0\times10^{-5}$ & $1.000010$ \\
$\Delta H_{\mathrm{rad}}$, $\beta_{\mathrm{ion}}=7.5\times10^{-2}$ & $+7.5\times10^{-2}$ & $0.985$ \\
$\Delta H_{\mathrm{mel}}$ at $M=1.44$ (seed 42) & $-0.720$ & $\mathbf{1.155}$ \\
\bottomrule
\end{tabular}
\end{table}

For one role of the two negatively signed species, the direct radiation term changes acceptance
by about one part in $10^5$; signed by role, its reach over source--target pairings is
$7.505\times10^{-2}$ in $\Delta H$. At the seed-42 field value the melanin term changes acceptance
by 15.5\%, about an order of magnitude beyond that reach (9.6 in $\Delta H$).\footnote{Corrected
2026-09-15. This sentence set the melanin figure against the one-role radiation value instead of
the term's reach, and did not say that the melanin figure comes from one seed; the field value, and
with it this ratio, varies between seeds (\texttt{PP-62-13}, \texttt{PP-65-06}).} Radiation can
therefore influence
...
(Figure~\ref{fig:melanin}). In this seed-42 run \textit{C.~sphaerospermum} reaches the largest
final field value, $M=1.44$, followed by \textit{C.~neoformans} and \textit{A.~niger}, which is the
order of the hand-specified production scales $\alpha_M$ ($0.14$, $0.10$, $0.065$). A single run
shows that this order occurred, not that the scales fix it, and at this configuration there are
seeds in which it inverts.\footnote{Corrected 2026-09-15. This sentence previously said the ordering
follows the hand-specified production scales, a statement about every run made from one
(\texttt{PP-65-06}).} These field values have no calibration to a physical melanin mass or
concentration.
...
within its radiation-derived pathway, the melanin term dominates the direct species-specific
radiation term by several orders of magnitude. The single-seed radial output does not establish a
...
is the dominant radiation-derived contribution to Metropolis acceptance, while the direct
species-specific radiation term is negligible for the two negatively signed classes. The resulting
...
\url{https://github.com/aurascoper/Biofilms}. The supporting repository revision for this
manuscript is \texttt{c7539a9}, the parent of the commit that adds this manuscript to the
repository: a document cannot cite the commit that contains it, so it names the revision it was
prepared against. No new biological specimens or experimental measurements were generated.
...
one synthetic-gate verdict was withdrawn as not resolution-converged. None of those numbers
appears in this manuscript, whose transport section reports the execution of a path rather than
its results. The corrections are recorded in \texttt{data/claims\_ledger.csv} and in
\texttt{docs/calibration/}. Three passages of this manuscript were themselves corrected after that
revision and carry a footnote where they occur: the abstract and the paragraph beneath
Table~\ref{tab:term_magnitudes} now scope the melanin acceptance figure to the single seed it was
computed at and set it against the radiation term's reach, and the melanin-field diagnostic no
longer presents the producers' order as a consequence of the production scales.
