...
measured results. The melanin term changes Metropolis acceptance by 13.4\% to 29.2\% across 256 seeds, whereas
...
Stating it here matters because the 13.4\% to 29.2\% acceptance shift reported in
...
$\Delta H_{\mathrm{mel}}$ over 256 seeds, $M=1.26$ to $2.56$ & $-1.281$ to $-0.630$ & $\mathbf{1.134}$ to $\mathbf{1.292}$ \\
\bottomrule
\end{tabular}
\end{table}

For the two negatively signed species \emph{occupying} a site, the direct radiation term changes
acceptance by about one part in $10^5$, which is not that term's reach, whereas the melanin term
changes it by 13.4\% to 29.2\% across seeds 42--297. That
...
and in the acceptance-favouring direction, smaller than the melanin term by a factor of $8.39$ to
$17.06$ in
...
half had already been fixed. \emph{Corrected a third time, 2026-09-17:} the shift read 15.5\% and
the factor read $9.6$. Both came from seed 42 alone. An independent Odin port ran seeds 42--297 at
the same configuration, and $M$ spans $1.2596$ to $2.5613$ there, with seed 42's $1.437157$ the
31st lowest of the 256. Radiation can therefore influence
...
$M=1.44$ at seed 42, followed by \textit{C.~neoformans} and \textit{A.~niger}. Over seeds 42--297
that value spans $1.26$ to $2.56$, and the ordering is \emph{corrected}: it fails in 19 of the 256
seeds, so it does not simply follow the
...
radiation term by about an order of magnitude in $\Delta H$ ($8.39$ to $17.06$ across 256 seeds), not by several. The single-seed radial output does not establish a
