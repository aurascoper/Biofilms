...
\begin{center}
  ...
  {\normalsize
    \textbf{Hunter Kinder}, B.A. --- Independent Researcher, Missouri, USA\\[2pt]
    \textbf{Brett Faulkner}, B.Sc. --- Independent Researcher\\[10pt]
  }
  {\small
    \textit{Preprint --- not peer reviewed}\\
    \textit{Version 1.2.1 --- September 2026}
  }
\end{center}
\vspace{4pt}
...
the direct per-species radiation term reaches $7.505\times10^{-2}$ in $\Delta H$ (a 1.5\% acceptance bias), about ten
times smaller than the melanin term rather than four orders smaller; only the product of melanin production and melanin
...
multispecies systems that exhibit porous-medium-type degeneracy. Our PSDE framework is specified to
extend these precedents with radiation-dependent fitness terms and phase-locking dynamics
coupling species interactions to external radiation fields, moving beyond the
substrate-limited growth paradigms that dominate existing biofilm models. Both additions remain
unexercised: Section~\ref{sec:psde} places the phase-locking adjustment among the four terms it
classes as unrepresentable, and the fitness field is reported there as absent from this
repository's simulation sources.
...
interaction topologies. Blasius et al.\ \cite{blasius1999} treat phase synchronization in
spatially extended ecological systems, which is the setting our phase-locking kernel
$\Gamma_s(t,\mathbf{x})$ is written for: it is specified within this tradition, extended to
multispecies microbial communities in which species-level oscillatory dynamics in growth and
resource acquisition would couple through shared radiation and nutrient fields. It remains
unexercised, and Section~\ref{sec:framework} places the phase-locking adjustment among the four
terms it classes as unrepresentable. The use of Langevin stochastic dynamics to describe microbial population
...
$H(p,q)$            & Hamiltonian function of this specification framework, defining total system energy. Distinct from the implemented $H_{\mathrm{CPM}}$ (Section~\ref{sec:cpm_term_magnitudes}), which is a Metropolis acceptance functional in arbitrary units carrying no absolute scale \\
...
$\omega_s$          & Phase frequency of species $s$ in $\phi_s(t)=\cos(\omega_s t-\theta_s)$ (Eq.~\ref{eq:adaptive}); no value is assigned, the adaptive term being unexercised (Section~\ref{sec:nutrient}) \\
...
\begin{itemize}\setlength\itemsep{2pt}
  ...
  \item \textbf{Hamiltonian interaction}: $\theta_s H_s(t,\mathbf{x})$, energy-conserving
        inter-species force from the specification Hamiltonian
        (Section~\ref{sec:hamiltonian}); the implemented $H_{\mathrm{CPM}}$ conserves nothing
        and is dissipated toward a minimum in the acceptance path.
...
\paragraph{The adaptive-feedback term is specification, not method.}
A nutrient field is integrated at every step by \texttt{update\_nutrient!}, so this uptake law
is not wholly unimplemented; the adaptive term $A_s(t)$ is the part that remains unexercised.
Its phase-alignment factor requires a phase variable, and Section~\ref{sec:psde} places the
phase-locking adjustment among the four terms it classes as unrepresentable, while the fitness
field $F_s$ in the saturation factor is reported there as absent from this repository's
simulation sources.
...
\paragraph{The name is a coinage, and it is not a description of the form.}
\emph{kNN} names an operation defined by a distance metric, a ranking and a cutoff $k$.
Eq.~\ref{eq:knn} has none of the three: it aggregates across the whole index range $j=1,\dots,n$
under the weight $P_{sj}$, which makes it a phase-weighted neighbour sum rather than a selection
among neighbours. It is likewise a single product with no split, so \emph{Decision Tree}
describes nothing in the form either. Section~\ref{sec:intro} holds each word to the endpoint it
names; that discipline was written for the five phenomenon terms, and it applies to method names
on the same grounds. The established description of this expression is a phase-weighted
neighbour sum over a transition matrix. $F_j$ is the fitness field of
Section~\ref{sec:psde}, reported there as absent from this repository's simulation sources, so
the quantity aggregated here is itself specified rather than implemented.
...
\emph{That last clause is now a theorem rather than a caution.} The Metropolis rule reads only
$\Delta H/T$, and each active term is linear in exactly one coefficient, so scaling every
coefficient \emph{and} $T_{\mathrm{CPM}}$ by a common factor leaves every acceptance decision
unchanged --- bit for bit, when the factor is a power of two. The coefficients and the
temperature are therefore identifiable only up to a common scale; no trajectory determines that
scale; and there is consequently no exchange rate between $J$ and Pascals for any fit to find.
That, rather than an appeal to model class, is why a bench modulus cannot be absorbed by fitting
coefficients. Ledgered as \texttt{SCALE-01} and asserted, with its inverted controls, in
\texttt{tests/hamiltonian\_scale\_invariance.jl}.

\emph{A tension, however, is not a modulus.} A contact energy per unit interfacial area is a
surface tension, so one measured tension together with a measured lattice pitch fixes the scale
and converts every coefficient of $H_{\mathrm{CPM}}$ (Section~\ref{sec:cpm_term_magnitudes}) to
absolute units at once. It is the one mechanical measurement this Hamiltonian as written can
absorb, and it fixes the scale \emph{only}: the self-adhesion diagonal is zero by construction,
so the differential-adhesion relation $\gamma_{ab}=J_{ab}-(J_{aa}+J_{bb})/2$ reduces to
$\gamma_{ab}=J_{ab}$, and a single cell--medium measurement sets the multiplier on the matrix
while determining none of its ratios. Registered as \texttt{D-TENSION} in
\texttt{data/calibration/reference\_d\_requirements.csv} --- \texttt{supplied\_by = measured},
\texttt{status = awaiting\_measurement} --- beside \texttt{D-PITCH} on the spatial gate.
...
is a reasonable simplification only while the sorbate is dilute and low-$Z$. \emph{The reason is the
interaction rather than the chemistry, and it carries its own boundary.} At the photon energies
of interest, attenuation through C, H, N and O is dominated by Compton scattering, which scales
with electron density per unit volume and is insensitive to atomic number, so two light-element
phases attenuate almost alike whatever their molecular identity. Hydrogen sharpens the point
rather than softening it: it carries the highest electron count per unit mass of any element, so
a hydrated organic phase and water converge instead of separating. The same mechanism fixes where
the condition lapses, and that boundary is checkable without any measurement of this system:
photoelectric absorption reasserts itself below roughly a few tens of keV for light elements, so
the simplification holds in the present energy regime and fails beneath it --- a statement about
the interaction rather than about this biofilm. The residual contrast between hydrated biomass
and its medium is a separate quantity and is left unstated here, since obtaining it requires the
water fraction and closed hydrated composition that Section~\ref{sec:limitations} lists among the
measurements this work still lacks. For a high-$Z$
...
\emph{pairings} of them, $7.505\times10^{-2}$, given in the text beneath Table~\ref{tab:decided_moves}.}
...
\emph{Corrected, version 1.2; see the correction note in the Software and Data Availability section.}
The direct radiation term was the sole decisive term in one accepted move of 206\,042 across seeds
42, 43 and 44 at 400~MCS (53\,603, 68\,465 and 83\,974 accepted moves respectively; Table~\ref{tab:decided_moves} shows seed 42) --- none under seeds 42 and 43, one under seed 44 --- and in none at
...
Measured over the three $N=40$ runs, seeds 42, 43 and 44 at 400~MCS, of which Table~\ref{tab:decided_moves} shows seed 42 --- $3\,968\,838$ evaluated proposals
--- $26.7\%$ carry
...
$N=20$; the three $N=40$ runs behind Table~\ref{tab:decided_moves} do not reach it, which is why the
...
% Separate labelled diagnostic; generated from the verified run manifest.
\input{label_trajectory_results}
\input{signal_field_results}
...
\url{https://github.com/aurascoper/Biofilms/tree/preprint-v1.2.1}. Version 1.0 was prepared against repository
revision \texttt{c7539a9}; this revision is prepared against \texttt{4bf1ba9}. Each names the
...
\emph{Correction, version 1.1: Figures~\ref{fig:radial} and~\ref{fig:melanin} have been replaced.} The versions distributed with
version 1.0 carried claims inside the images that the surrounding text retracts. Figure~\ref{fig:melanin}
...
Section~\ref{sec:radiotrophy_status}; Figure~\ref{fig:radial} labelled a shaded band a ``radiotrophic niche'',
...
regenerated. Figure~\ref{fig:contaminant} still carries a ``per cent depleted'' label that should read
non-penetration, and is not regenerated here: its quantities are downstream of the gated biomass
basis. Figure~\ref{fig:membrane} is not --- it plots $m(t)$ and $P_{\mathrm{eff}}/P_0$, neither of which reads that
...
by Codex on pull request \#23. Figure~\ref{fig:membrane} has separately been rebuilt to remove an in-plot
...
\url{https://doi.org/10.3389/fmicb.2020.00560}
...
\url{https://doi.org/10.3389/fmicb.2020.587556}
...
\url{https://doi.org/10.3389/fmicb.2023.1261801}
