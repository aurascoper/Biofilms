...
\begin{abstract}
\noindent
We present a computational framework and implementation audit for multispecies biofilm models under
heterogeneous ionising-radiation fields. A proposed partial stochastic differential equation
(PSDE) organises diffusion, directed motility, nutrient, radiation, melanin, interaction and
forcing terms, while four executed simulations---two Langevin models, a Cellular Potts Model
(CPM), and a radial radiodialysis solver---realise only subsets of that formalism. The CPM represents
seven named species as computational biomass parcels rather than individual organisms. In the
shipped parameterisation, the melanin term changes Metropolis acceptance by approximately
15.5\%, whereas the direct per-species radiation term changes it by approximately one part in
$10^5$ for the two negatively signed species; only the product of melanin production and melanin
coupling reaches the dynamics and is identifiable. A single-seed radial pattern is opposite to the
direction implied by the direct tropism coefficient and is not clearly separated from the approximate seeding
null with six parcels per species. A synthetic, explicitly non-calibrating CPM--OpenMC workflow
executes one-way photon transport, constructive-solid-geometry mass normalisation, dose conversion,
coordinate transfer, and cell-, lineage- and generation-level attribution, but no target physical
parameter is inferred. The implemented biological phenotype is therefore radiotropism, not
radiotrophy or radioresistance: the current CPM has neither biomass growth nor a survival process.
The work establishes a reproducible computational foundation and identifies the measurements,
uncertainty controls, and model revisions required before physical or biological feedback claims can
be evaluated.
\end{abstract}
...
constants $I_0=1.0$ and $T_{\mathrm{CPM}}=5.0$.}
