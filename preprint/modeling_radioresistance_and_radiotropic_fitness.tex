...
Xavier et al.\ \cite{xavier2005} is a different construction --- a hybrid individual-based
framework for the activity and structure of multispecies biofilms --- and belongs with the
individual-based work below rather than with the continuum models. Individual-based modeling
...
\emph{Two positive results run against that reading and are cited here rather than left out
of it.} Khajo et al.\ \cite{khajo2011} report that melanin protects
\textit{Cryptococcus neoformans} from lethal-dose gamma irradiation through sacrificial
breakdown of the pigment, and Malo et al.\ \cite{malo2018} report a protective effect of
melanin on \textit{C.\ neoformans} morphology after both sparsely and densely ionizing
radiation. \textit{C.\ neoformans} is one of the seven species modelled here, so these are
same-species evidence for the assumption the preceding sentences decline, and this
repository's own evidence audit already files the second under
\texttt{melanized\_radioprotective} --- a category defined earlier in this section and, until
now, never populated in the text. The isogenic null results and these positives are not
reconciled here, and stating both is the point: a section whose argument rests on naming what
runs against its position cannot omit the two results that do.
...
implied: this model has no route by which morphology could arise, and its spatial output is
therefore directional redistribution of parcels in an imposed field, not a morphology.
...
Bioremediation of radioactive waste: radionuclide--microbe interactions in laboratory and
field-scale studies.
