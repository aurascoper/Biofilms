...
function signal_references(path)
    ...
    # update_signal! stays outside the \b...\b group: that trailing \b binds to the whole
    # alternation and `!` is a non-word character, so `update_signal!(` -- every real call
    # form -- would never match. Do not tidy it back in.
