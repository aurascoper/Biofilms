...
| the manuscript calls it symmetry, not a wall | `preprint/modeling_radioresistance_and_radiotropic_fitness.tex:804` | `Zero-flux symmetry is imposed at $r = 0$.` |
...
| the manuscript claims both ends are in the operator | `preprint/modeling_radioresistance_and_radiotropic_fitness.tex:1068` | `represented explicitly in the semi-discrete operator` |
...
| the Julia port substeps against the diffusion bound | `biofilms_potts.jl:1474` | `dt_stable = 0.4 * dr^2 / (2.0 * rd.params.D_eff)` |
