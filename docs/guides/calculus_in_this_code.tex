...
\begin{panelbox}
\textbf{What this is.} The mathematics the radiodialysis solver actually contains, derived
rather than named.

\textbf{What is mechanically checked, and it is not everything.} Each part ends with a
\emph{Checks against the source} table. Every row there carries a \texttt{file:line} and a
fragment that line must contain, and \texttt{calibration/tests/test\_guide\_citations.py}
verifies all of them — a citation that has gone stale fails the suite, and the failure says
whether the code moved or the code changed. \textbf{Claims made in the prose but not
appearing in a table are not mechanically verified.} Three in this section are worth naming
as such: the reading of \texttt{bc\_coef} as $P_{\text{eff}}$ versus $k_L$, the sorption
rates quoted in Part 5, and the stiffness characterisation in Part 5. All three are sourced
in the text; none is pinned by a test. Saying so is the point — a guide that implied uniform
coverage would be making the claim its own tables exist to discipline.

\textbf{Floor.} The chain rule, the product rule, and what a partial derivative is.

\textbf{Ceiling.} Ordinary differential equations as objects an integrator solves. No vector
...
\textbf{Relationship to the README.} \texttt{README.md} has a \emph{Mathematical framework}
section that states the project's \emph{target formalism} — under its own disclaimer that
what the programs integrate differs. This guide is the other side of that: it describes
\textbf{what the code integrates}, and it cross-references those equations rather than
restating them, so the two cannot drift into disagreeing.

\textbf{Deliberately out of scope:} general curvilinear coordinates, the divergence theorem
in 3D, stability proofs, and the sorption terms beyond noting that they set a second
...
\textbf{And the physical content is real.} Flux converging on a shrinking face concentrates;
...
\textbf{The resolution is symmetry.} The model is radially symmetric — $c$ depends on
...
\textbf{This is why the manuscript calls it symmetry rather than a wall.} The condition at
...
the manuscript calls it symmetry, not a wall
  & preprint/modeling\us radioresistance\us and\us radiotropic\us fitness.tex:660
...
\textbf{So the code is a hybrid, and the contrast is the useful thing.} Flux-form in the
...
the manuscript describes both ends as handled in the operator
  & preprint/modeling\us radioresistance\us and\us radiotropic\us fitness.tex:912
...
\textbf{It interpolates between the two conditions you already know.}
...
\textbf{Getting from that statement to the code is one substitution.} Introduce a ghost node
...
\textbf{One naming caution the code is emphatic about.} \texttt{bc\_coef} is
...
\textbf{Why not just step it forward yourself.} An explicit step is limited by stability, not
...
\textbf{And that is only one of the timescales.} Sorption runs at its own rate — an uptake of
...
\textbf{That is what LSODA is for.} It monitors the solution and switches between an Adams
...
LSODA is the integrator
  & biofilms\us radiodialysis.R:380
  & method = "lsoda" \\[0.35em]
the Julia port substeps against the diffusion bound
  & biofilms\us potts.jl:1444
